...
**************************************/
/*
	Unfortunately I couldn't find a way to move this array to the bottom 
 ...
 ***************************************************************/
 /*
 	This function creates a window in four basic steps. 
	 1. It determines the size of the windows frame and adjusts the frames position if 
	 it is outside the bounds of the screen.
	 2. It checks to see if there is room for another window to be 
	 created in the windows array.
	 3. It then assigns the windows attributes to the window management struct.
	 4. Finally, it draws the window by calling the draw_frame() function.
 */
 ...
 * **********************************************/
 /*
	this function makes sure a window exists. 
	If it does, it calls the set_pixel function
 */
 ...
 * **********************************************/
 /*
	In order to divide up the labor of drawing characters to the screen, 
	I created a series of helper functions. This function basically makes 
	sure the window is valid and then passes the work off to the draw_string() funtion.
	The draw_string() function the passes one character at a time to the 
...
/*
	Draw frame first draws a large white rectangle, then a smaller black rectangle which 
	will be the canvas, then it writes the title on the frame.
*/
void draw_frame (window * wnd) 
{
	/* render window */
...
/*
	Fill-rect() just draws a rectangle onto the screen, it does not call set_pixel because the dimensions of the 
	frame are checked inside the create window function. It cannot draw outside the vga sector of memory.
*/
...
/*
	Set_pixel() checks the assigned bounds attribute for clipping, then pokes the pixel into memory.
*/
void set_pixel (window * wnd, int x, int y, int color, int bounds) 
{
    switch (bounds)
...
/*
	This is almost identical to the poke_screen function from window.c
*/
...
/*
	One of the helper functions for draw_text(). It reads a string one char at a time 
	and passes the instructions to the draw character.
	The x coordinate must be incremented by 8 each time because that is the width 
...
/*
	It grabs a hex value from the g_8x8_font array and reads one bit at a time. 
	if it is 0 it draws bg_color, if 1 it draws fg_color. This is done a total of 8 times.
*/
...
/*
	Simple clip check, the bounds attribute is what determines the context of the height/width. 
	See set_pixel() for a better description.
*/
int clip_check (int x, int y, int width, int height)
{
...
/*
	Used for drawing the straight line. I didnt know if it was aleady 
	imlemented in TOS or not so I just made this to be sure .
*/
int m_abs (int a) {
	return a < 0 ? -a : a;
}
